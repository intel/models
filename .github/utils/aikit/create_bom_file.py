...
def generate_bom_file(bom_file_name, drop_delivery_dir, component):
    ...
    header = (
        "DeliveryName\tInstallName\tFileCheckSum\tFileOrigin\tInstalledFilePermission\n"
    )
...
def main():
    ...
    parser.add_argument(
        "--drop_delivery_dir",
        help="component binary drop directory",
        type=str,
        required=True,
    )
    parser.add_argument(
        "--boms_dir",
        help="directory where generated bom files will be stored",
        type=str,
    )
    parser.add_argument(
        "--component", help="component name", type=str, default="modelzoo"
    )
