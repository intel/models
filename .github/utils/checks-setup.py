...
def main(args):
    ...
    if args.subcommand == "pr-check":
        ...
        print(
            "Invalid subcommand provided. Use pr-check|health-check subcommands. Exiting..."
        )
...
def setup_runners():
    ...
    runner = {"cpu": "k8-runners", "gpu": "max:1550"}
    ...
    for file in pr_info[:]:
        if file["status"] in ["added", "modified", "renamed", "copied", "changed"]:
            ...
            if (
                ...
                checks_json["files"]["bom"].append(
                    build_filename_diff_url(file["filename"], url)
                )
            # model change
...
def build_filename_diff_url(filename, url):
    file_info = {"filename": f"{filename}", "diff_url": ""}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f",
        "--test_file",
        help="Yaml file name that contains the tests for the models",
        default="tests.yaml",
    )
    parser.add_argument(
    ...
    hchk_parser = subparsers.add_parser(
        "health-check",
        help="Create the JSON checks config for health check on the repository",
    )
    prchk_parser = subparsers.add_parser(
        "pr-check", help="Create the JSON checks config for PR check workflow"
    )
    prchk_parser.add_argument(
