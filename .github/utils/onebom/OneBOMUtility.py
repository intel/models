...
class OneBOMUtility:
    ...
    def __init__(self):
        ...
        parser.add_argument('-g', '--get-guid', action='store_true', help='Pass this flag to get the GUIDs for the given IPX names')
        parser.add_argument('-r', '--requirements', required=True, type=str, default=None, help='Path to the file with a list of requirements files')
        parser.add_argument('-o', '--output', type=str, default=None, help='Output file to write the results to')
        self.args = None
    ...
    def ipx_ip_name_from_requirements_path(self, req_path):
      ...
      app_name = req_path[OneBOMGlobals.ROOT_STR_LEN:-OneBOMGlobals.REQUIREMENTS_STR_LEN]

      # Replace the "/" with "_"
      if len(app_name) > 0:
        ip_name = f"{OneBOMGlobals.AIRM_IPX_PREFIX}-{app_name.replace("/", "-")}_src"
      else:
        ip_name = f"{OneBOMGlobals.AIRM_IPX_PREFIX}_src"

      OneBOMGlobals.debug (f"ipx_ip_name_from_requirements_path: {req_path} -> {ip_name}")
      return ip_name
    ...
    def parse_and_validate_args(self, parser): 
        ...
        if not self.args.requirements or not os.path.exists(self.args.requirements):
            OneBOMGlobals.error(f"List of reuirements.txt files in {self.args.requirements} does not exist.")
    ...
    def run(self):
        ...
        try:
            ...
            import sys
            OneBOMGlobals.error(f"IPXUtils not found. Please make sure the IPXUtils module is available: {e.msg}. Current PYTHONPATH: {sys.path}")
        ...
        for file in requirements:
            ...
            if self.args.get_guid:
                guid = ipxUtils.get_guid_of_latest_version(OneBOMGlobals.AIRM_IP_LIBRARY, name)
