...
def generate_app_lists(
    new_collection, old_collection, pinned_file_name="requirements.txt"
):
    ...
    apps_needing_disabled = (
        list()
    )  # this will NOT be automatic without specific approval - for now it just collects for artifacts
...
def package_app_lists_for_artifacting(collection, path_to_package_directory):
    ...
    for item in collection:
        ...
        shutil.copytree(
            item["path_to_metadata"],
            f"{path_to_package_directory}/" + item["index"],
            dirs_exist_ok=True,
        )
...
def args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="path to root directory for previous release metadata",
    )
    parser.add_argument(
        "-n",
        "--next-release",
        type=str,
        required=True,
        help="path to root directory for next release metadata",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        required=True,
        help="path to output directory packages should be collected (directory structure should exist)",
    )
...
if __name__ == "__main__":
    ...
    (
        apps_needing_created,
        apps_needing_updated,
        apps_needing_disabled,
    ) = generate_app_lists(
        next_release_metadata, previous_release_metadata, "requirements.txt"
    )
    package_app_lists_for_artifacting(
        apps_needing_created, f"{output_dir}/apps-needing-create"
    )
    package_app_lists_for_artifacting(
        apps_needing_updated, f"{output_dir}/apps-needing-update"
    )
    package_app_lists_for_artifacting(
        apps_needing_disabled, f"{output_dir}/apps-needing-disable"
    )
