...
def generate_list_of_requirements_files(root_directory, output_file):
    requirements_files_list = [
        x.replace(f"{root_directory}/", "")
        for x in glob.glob(f"{root_directory}/**/requirements.txt", recursive=True)
    ]
...
def args():
    ...
    parser.add_argument(
        ...
        default=".",
        required=False,
        help="path to root directory for requirements.txt discovery",
    )
    parser.add_argument(
        "-o",
        "--output",
