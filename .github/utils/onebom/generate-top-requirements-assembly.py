    ...
    with open(path_to_requirements_file, 'r') as fh:
        ...
        for item in raw_data:
            if '@' in item:
                ...
                continue
            requirements_version = list(filter(None, re.split(regex_pattern , item))) # search for two characters to split on
...
def generate_frozen_master_dependencies(path_to_requirements, path_to_frozen, output_file):
...
def args():
    ...
    parser.add_argument('-r', '--requirements', type=str, required=True, help='path to root requirements file (pre-pip-freeze)')
    parser.add_argument('-f', '--frozen', type=str, required=True, help='path to frozen requirements file (post-pip-freeze)')
