...
class OneBOMSpeedReleaseFactory:
    ...
    def __init__(self):
        ...
        parser = argparse.ArgumentParser(
            description="Creates OneBoM SPEED items from apps-needing-variant.txt and apps-needing-release.txt."
        )

        # Add the arguments
        parser.add_argument(
            "-v",
            "--apps-needing-variants",
            type=str,
            default=None,
            required=False,
            help="path to the apps-needing-variant.txt file",
        )
        parser.add_argument(
            "-r",
            "--apps-needing-release",
            type=str,
            default=None,
            required=False,
            help="path to the apps-needing-variant.txt file",
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="The release variant information, e.g. v3.1.1",
        )
        parser.add_argument(
            "-d",
            "--dry-run",
            action="store_true",
            help="Don't actually create the items, just print what would be done.",
        )
        parser.add_argument(
            "-t", "--use-token", action="store_true", help="Use OAUTH Token."
        )
        parser.add_argument(
            "-w",
            "--release-workload-collection",
            type=str,
            default=None,
            required=False,
            help="path to the list of completed Release item codes that need to be added to the workload collection",
        )
        parser.add_argument(
            "-l",
            "--log-level",
    ...
    def parse_and_validate_args(self, parser):
        ...
        if self.args.log_level:
            ...
            DEBUG = True
        if self.args.apps_needing_variants and not os.path.exists(
            self.args.apps_needing_variants
        ):
            error(f"{self.args.apps_needing_variants} does not exist.")

        if self.args.apps_needing_release and not os.path.exists(
            self.args.apps_needing_release
        ):
        ...
        if self.args.release_workload_collection:
            if os.path.exists(self.args.release_workload_collection):
                debug(
                    f"Release Workload Collection file: {self.args.release_workload_collection}"
                )
                ...
                print(
                    f"{self.args.release_workload_collection} does not exist. Generating automatically."
                )
    ...
    def create_variant(self, app_path):
        ...
        if self.args.dry_run:
            ...
            response = self._onebom_speed_reqest_manager.put(
                OneBOMSpeedRequestManager.ITEM_ENDPOINT, data=speed_item.dumps()
            )
    ...
    def create_release(self, app_path, parent_item_code):
        speed_item = OneBOMSpeedRelease(
            parent_item_code, app_path, self.args.release_info
        )
        response = self._onebom_speed_reqest_manager.put(
            OneBOMSpeedRequestManager.ITEM_ENDPOINT, data=speed_item.dumps()
        )
    ...
    def create_releases(self):
        with open(self.args.apps_needing_release, "r") as f:
            for line in f:
                ...
                parent_item_code = self.get_existing_item_code(
                    OneBOMSpeedItem.VARIANT, app_path
                )
                if self.args.dry_run:
                    print(
                        f"Would create release for {app_path} with parent variant code {parent_item_code}"
                    )
                else:
                    speed_item = OneBOMSpeedRelease(
                        parent_item_code, app_path, self.args.release_info
    ...
    def create_workload_collection_release(self):
        ...
        speed_item = OneBOMSpeedRelease(
            OneBOMSpeedItem.AIRM_WORKLOAD_COLLECTION_VARIANT_ITEM_CODE,
            "",
            self.args.release_info,
        )
        if self.args.dry_run:
            print(
                f"""Would create workload-collection release {self.args.release_info}:
            ...
            debug(
                f"Workload Collection Release {self.args.release_info} Item Code: {workload_collection_release_item_code}"
            )
            # build the BoM using all the releases
            for bom_item_code in self._release_bom_items:
                bom_item = OneBOMSpeedReleaseBOMItem(
                    workload_collection_release_item_code, bom_item_code.split(",")[1]
                )
                response = self._onebom_speed_reqest_manager.post(
                    OneBOMSpeedRequestManager.BOM_ENDPOINT, data=bom_item.dumps()
                )
                if response is not None and response.status_code == 200:
                    print(
                        f"workload-collection BoM addition: SUCCESS!!!! {bom_item_code}"
                    )
                else:
                    error(
                        f"workload-collection BoM: FAILURE!!!! Status: Item Code: {bom_item_code}; {response.status_code}; Text: {response.text}"
                    )
